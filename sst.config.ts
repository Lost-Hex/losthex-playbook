...
/**
 * SST v3 configuration for LostHex Base
 * Uses OpenNext for advanced Next.js features and optimal serverless deployment
 */
export default $config({
  app(input) {
    ...
    return {
      ...
      // e.g. "losthex-dev-Site" for dev stage, "losthex-prod-Site" for prod
      name: "losthex-base",
  ...
  async run() {
    ...
    // OpenNext provides better support for:
    // - Middleware at edge
    ...
    const site = new sst.aws.Nextjs("Site", {
      ...
      // - Ephemeral branches: Use AWS managed "CachingDisabled" policy (doesn't count against quota)
